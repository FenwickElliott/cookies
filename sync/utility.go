...
func insert(nativeID, partner, partnerCookie string) error {
	...
	var res bson.M
	err = service.c.Find(bson.M{"_id": nativeID, partner: partnerCookie}).One(&res)
	if err == nil {
		return nil
	}

	err = service.c.FindId(nativeID).One(&res)
	if err == nil {
		err = service.c.UpdateId(nativeID, bson.M{"$set": bson.M{partner: partnerCookie}})
	} else if err.Error() == "not found" {
		err = service.c.Insert(bson.M{"_id": nativeID, partner: partnerCookie})
	} else {
		return err
	}
...
func getOrSetCookie(w *http.ResponseWriter, r *http.Request) *http.Cookie {
	...
	if nativeCookie == nil {
		...
		var res bson.M
		err = service.c.Find(bson.M{partner: partnerCookie}).One(&res)
		if err == nil {
